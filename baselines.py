...
def train_tailgan_static(
    ...
    D = Discriminator(config.d_hidden_dims, config.dropout_rate, use_sigmoid=True).to(device)
    ...
    train_loader = prepare_dataloader(data_train, config.batch_size, device, shuffle=True)
    ...
    for epoch in range(config.epochs):
        ...
        if epoch % 10 == 0:
            ...
            val_metrics = compute_tail_metrics(data_val, gen_val, config.tail_threshold_q)
...
def train_importance_sampling_baseline(
    data_train: np.ndarray,
    n_samples: int = 10000,
    tail_oversample_factor: float = 3.0,
    threshold_q: float = 0.05,
    seed: int = 42
) -> np.ndarray:
    """
    FIX Issue 18: Importance sampling baseline - oversample tail manually.
    
    Args:
        data_train: Training data
        n_samples: Number of samples to generate
        tail_oversample_factor: How much to oversample tail
        threshold_q: Tail threshold
        seed: Random seed
    
    Returns:
        Samples with oversampled tail
    """
    np.random.seed(seed)
    
    thresh = np.quantile(data_train, threshold_q)
    tail_data = data_train[data_train <= thresh]
    body_data = data_train[data_train > thresh]
    
    # Oversample tail
    n_tail_expected = int(n_samples * threshold_q)
    n_tail_actual = int(n_tail_expected * tail_oversample_factor)
    n_body = n_samples - n_tail_actual
    
    if n_body < 0:
        n_body = n_samples // 2
        n_tail_actual = n_samples - n_body
    
    tail_samples = np.random.choice(tail_data, size=n_tail_actual, replace=True)
    body_samples = np.random.choice(body_data, size=n_body, replace=True)
